...
Purpose: Add round history with a 2D array and vectors passed to functions
*/
...
void   shop(int &, bool &, const vector<string> &,
            const vector<int> &);             //Open the game shop
...
void   prnHis(const int [][2], int);          //Print 2D round history
vector<string> mkItem();                      //Return shop item vector
vector<int>    mkCost();                      //Return shop cost vector
...
int main(int argc, char** argv)
{
    ...
    int hist[MAXRND][2];       //Two dimensional array holding round cards
    ...
    vector<string> items;      //Vector holding shop item names
    vector<int> costs;         //Vector holding shop item costs
    ...
    //Create the shop vectors using returned vectors
    items = mkItem();
    costs = mkCost();
    ...
    do
    {
        ...
        while(i <= rounds)
        {
            ...
            //Store both cards in the two dimensional history array
            hist[i - 1][0] = youCard;
            hist[i - 1][1] = cpuCard;
        ...
        //Display the 2D round history through a function
        prnHis(hist, rounds);
        ...
        //Open the shop and pass both vectors to the function
        if(visit == 'y' || visit == 'Y')
        {
            shop(coins, boost, items, costs);
...
int drawCrd(int deck[], int &pos, int size)
{
    ...
    //Return to the start if the deck reaches its end
    if(pos >= size)
    {
        pos = 0;
    }
...
//Allow the player to use the shop vectors to make a purchase
void shop(int &coins, bool &boost,
          const vector<string> &items, const vector<int> &costs)
{
    int choice;

    //Display the shop menu using the vectors
    cout << endl;
    cout << "WAR SHOP" << endl;
    ...
    for(int i = 0; i < static_cast<int>(items.size()); i++)
    {
        ...
        //Display a cost only when the item has one
        if(costs[i] > 0)
        {
            cout << ", " << costs[i] << " coins";
        }
...
int linSrc(const string names[], int size, string target)
{
    ...
    while(i < size && pos == -1)
    {
        //Compare returns zero when both names match
...
//Print card values stored in the two dimensional history array
void prnHis(const int hist[][2], int rounds)
{
    cout << endl;
    cout << "Round History" << endl;

    //Print both cards from each completed round
...
//Create and return the vector containing shop item names
vector<string> mkItem()
{
    vector<string> items;

    //Add each shop menu item to the vector
    items.push_back("Winning bet booster");
    items.push_back("Leave shop");
    items.push_back("Quit game");

    return items;
}

//Create and return the vector containing shop item costs
vector<int> mkCost()
{
    vector<int> costs;

    //Add the matching cost for each shop item
    costs.push_back(25);
    costs.push_back(0);
    costs.push_back(0);
